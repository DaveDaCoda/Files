    ...
    public class Interaction
    {
        ...
        private async void OpenSelectedItems(bool displayApplicationPicker)
        {
            ...
            if (selectedItemCount == 1)
            {
                ...
                {
                    var res = await AssociatedInstance.FilesystemViewModel.GetFolderWithPathFromPathAsync(
                ...
                {
                    var res = await AssociatedInstance.FilesystemViewModel.GetFileWithPathFromPathAsync(clickedOnItem.ItemPath);
                    if (res)
                    {
        ...
        public async Task<bool> RenameFileItem(ListedItem item, string oldName, string newName)
        {
                ...
                && !ContainsRestrictedFileName(newName))
            {
                ...
                {
                    var res = await AssociatedInstance.FilesystemViewModel.GetFolderFromPathAsync(item.ItemPath);
                    renamed = res ? await res.Result.RenameAsync(newName, NameCollisionOption.FailIfExists).AsTask().Wrap() : res;
                }
                else
                {
                    var res = await AssociatedInstance.FilesystemViewModel.GetFileFromPathAsync(item.ItemPath);
                    renamed = res ? await res.Result.RenameAsync(newName, NameCollisionOption.FailIfExists).AsTask().Wrap() : res;
                }
            ...
            if (AssociatedInstance.ContentPage.IsItemSelected)
            {
                foreach (ListedItem listedItem in AssociatedInstance.ContentPage.SelectedItems)
                {
                    try
                    {
                        ...
                        {
                            StorageFile file = await AssociatedInstance.FilesystemViewModel.GetFileFromPathAsync(recycleBinItem.ItemPath);
                            StorageFolder destinationFolder = await AssociatedInstance.FilesystemViewModel.GetFolderFromPathAsync(Path.GetDirectoryName(recycleBinItem.ItemOriginalPath));
                        ...
                        StorageFile iFile = await AssociatedInstance.FilesystemViewModel.GetFileFromPathAsync(iFilePath);
                        if (iFile != null)
                        {
        ...
        public async void CutItem_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (AssociatedInstance.ContentPage.IsItemSelected)
            {
                ...
                foreach (ListedItem listedItem in AssociatedInstance.ContentPage.SelectedItems)
                {
                    ...
                    {
                        var res = await AssociatedInstance.FilesystemViewModel.GetFileFromPathAsync(listedItem.ItemPath);
                        cut = res;
                        if (!res) break;
                    ...
                    {
                        var res = await AssociatedInstance.FilesystemViewModel.GetFolderFromPathAsync(listedItem.ItemPath);
                        cut = res;
                        if (!res) break;
                        items.Add(res.Result);
                    }
